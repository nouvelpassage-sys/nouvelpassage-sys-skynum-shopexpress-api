...
export function improveShortDescription({ description, name, productType, category }) {
  ...
  if (String(category).includes("Гарячі пропозиції")) {
    return `${capitalize(type)} зберігає відчуття преміальної флористики, але подається як спеціальна пропозиція. Важливо, що це не компроміс у стилі, а готовий варіант для швидкого вибору красивого подарунка.`;
  }

  if (String(category).includes("Декор та подарунки")) {
    return `${capitalize(type)} працює як окрема деталь у замовленні: додає фактуру, колір або практичний акцент без зайвого пафосу. Опис має спиратися на реальний предмет, його матеріал, форму або функцію, а не на загальні слова про подарунок.`;
  }
